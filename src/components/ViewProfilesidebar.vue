<template>
  <div class="sidebar-container">
    ...
    <h2 class="name" >{{ user.name ? user.name : 'Unknown'}}</h2>
    <h3 class="username">@ {{ user.username ? user.username : 'Unknown' }}</h3>
    ...
    <div class="links-section">
      ...
      <div v-if="user.social.facebook" class="social-link">
        ...
        <span>{{ user.social.snapchat }}</span>
      </div>
      <div v-if="user.social.facebook" class="social-link">
        <font-awesome-icon :icon="['fab', 'tiktok']" />
        <span>{{ user.social.tiktok }}</span>
      </div>
      <div v-if="user.social.facebook" class="social-link">
        <font-awesome-icon :icon="['fab', 'twitch']" />
        <span>{{ user.social.twitch }}</span>
      </div>
      <div v-if="user.social.facebook" class="social-link">
        <font-awesome-icon :icon="['fab', 'youtube']" />
        <span>{{ user.social.youtube }}</span>
      </div>
    </div>
...
export default {
  data() {
    return {
      user: null,
      ethAddress: this.$route.params.id,
      showToolTip: false,
      userProfileImg: null,
    };
  },
  async created() {
    ...
    let viewUser = await this.axios.post(
    ...
    this.user = viewUser.data.user
    console.log(this.user, 'the userside bar');
    this.userProfileImg = this.user.profileImage;
  },
  methods: {
    shortenAddress() {
      return (
        this.ethAddress.substring(0, 6) +
        "..." +
        this.ethAddress.substring(this.ethAddress.length - 4)
      );
    },
    copyToClipBoard() {
...
.profile-avatar {
  ...
  background-image: linear-gradient(#2f3030, #2f3030),
    linear-gradient(to bottom, #ae9cf8, #e5c3c3);
